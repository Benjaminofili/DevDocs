// src/lib/ai/providers/multi-provider.ts
    ...
    async generate(prompt: string, context?: string): Promise<AIResponse> {
        ...
        for (const provider of this.providers) {
            if (!provider.isConfigured()) {
                console.log(`⚠️ ${provider.constructor.name} not configured, skipping...`);
                continue;
            }

            try {
                console.log(`🔄 Trying ${provider.constructor.name}...`);
                const response = await provider.generate(prompt, context);
                console.log(`✅ Success with ${provider.constructor.name}! Provider: ${response.provider}`);
                return response;
            } catch (error: any) {
                ...
                if (errorMessage.includes('rate limit') || errorMessage.includes('quota') || errorMessage.includes('429')) {
                    console.log(`🚫 ${provider.constructor.name} rate limited, trying next provider...`);
                    continue;
                }
                
                // For other errors, also try next provider
                console.log(`❌ ${provider.constructor.name} failed: ${errorMessage}`);
            }
        }
